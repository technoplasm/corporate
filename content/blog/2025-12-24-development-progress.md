...
例えば80commit付近の大きなローソクは、誤って生成されたコードをコミットして削除したところにあたります。

横軸はコミット単位で時間の長さには対応しないので、時間軸で出力できるようにしても面白いかもしれません。

チャート作成に使ったスクリプトはほぼバイブコーディングしたまま中身は禄に見てませんが、いちおうGitHubに上げておきました（[technoplasm/git-candlestick](https://github.com/technoplasm/git-candlestick) ）。良ければご覧ください。
